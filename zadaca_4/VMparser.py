class Parser:
    ...
    def _push(self, src, loc, n):
        if loc not in "0123456789":
            if loc > 32767 or loc < 0:
                self._flag = False
                Parser._error("push", n, "Undefine location \"" + loc + "\".")
                return ""
        elif src == "constant":
            l = "@" + str(loc) + "\nD=A\n"
        elif src == "local":
    ...
    def _pop(self, dst, loc, n):
        if loc not in "0123456789":
            if loc > 32767 or loc < 0:
                self._flag = False
                Parser._error("pop", n, "Undefine location \"" + loc + "\".")
                return ""
        elif dst == "local":
            l = "@" + str(loc) + "\nD=A\n@LCL\nD=D+M\n@R15\nM=D\n@SP\nAM=M-1\nD=M\n@R15\nA=M\nM=D"
        elif dst == "argument":
    ...
    def _ifgoto(self, lab, n):
        ...
        if (label not in self._labels):
            self._flag = False
            Parser._error("ifgoto", n, "Undefined label \"" + label + "\".")
            return ""
        return "@SP\nAM=M-1\nD=M+1\n@" + self._func + "$" + lab + "\nD;JEQ"
